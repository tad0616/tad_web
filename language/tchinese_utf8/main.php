...
define('_MD_TCW_GOOD_LOGO_SITE', "<ol><li>可利用 <a href='https://www.canva.com' target='_blank'>https://www.canva.com</a> 線上製作 logo 圖。</li><li>若 logo 圖拖曳到看不見，<a href='config.php?WebID=%s&op=reset_logo'>可點此恢復其預設位置</a>。</li><li>若看不到新圖或調整後結果，九成九是您的瀏覽器快取在做怪，請按 <kbd>Ctrl</kbd>+<kbd>F5</kbd> 試試。</li></ol>");
    ...
    if ($dh = opendir($dir)) {
        while (false !== ($file = readdir($dh))) {
            if ('dir' === filetype($dir . $file)) {
                ...
                require_once XOOPS_ROOT_PATH . "/modules/tad_web/plugins/{$file}/langs/tchinese_utf8.php";
            }
        }
        closedir($dh);
    }
...
define('_MD_TCW_NO_SPACE', '您（%s）已使用 %s MB的硬碟空間，已達上限 %s MB，無法再新增資料。');
...
define('_MD_TCW_RE_GENERATE_SCREEN', '重新產生畫面');
define('_MD_TCW_ADM_DEFALTWEBNAME', '（因為您是管理員，所以底下為「%s」的選單）');
