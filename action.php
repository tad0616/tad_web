...
$xoopsOption['template_main'] = "tad_web_action_tpl.html";
...
function tad_web_action_form($ActionID=""){
  ...
  if(!$isMyWeb and $MyWebs){
    redirect_header($_SERVER['PHP_SELF']."?op=tad_web_action_form&WebID={$MyWebs[0]}",3, _MD_TCW_AUTO_TO_HOME);
  }elseif(empty($MyWebs)){
...
function insert_tad_web_action(){
	...
	values('{$_POST['ActionName']}' , '{$_POST['ActionDesc']}' , '{$_POST['ActionDate']}' , '{$_POST['ActionPlace']}' , '{$uid}' , '{$_POST['WebID']}' , '{$_POST['ActionCount']}')";
...
$ActionID=(empty($_REQUEST['ActionID']))?"":intval($_REQUEST['ActionID']);
...
switch($op){
  ...
  header("location: {$_SERVER['PHP_SELF']}?ActionID=$ActionID");
  break;
