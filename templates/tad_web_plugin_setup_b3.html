    ...
    <div class="form-group">
      ...
      <div class="col-sm-4">
        ...
        <{elseif $setup.type=="radio"}>
            <{foreach from=$setup.options key=title item=value}>
              <label class="radio-inline">
                <input type="radio" name="<{$setup.name}>" id="<{$setup.name}>" value="<{$value}>" <{if $setup.value==$value}>checked<{/if}>><{$title}>
              </label>
            <{/foreach}>
        <{elseif $setup.type=="checkbox"}>
            <{foreach from=$setup.options key=title item=value}>
              <label class="checkbox-inline">
                <input type="checkbox" name="<{$setup.name}>[]" id="<{$setup.name}>_<{$value}>" value="<{$value}>" <{if $value|in_array:$setup.value}>checked<{/if}>><{$title}>
              </label>
            <{/foreach}>
        ...
        <{elseif $setup.type=="yesno"}>
          <label class="radio-inline">
            <input type="radio" name="<{$setup.name}>" id="<{$setup.name}>1" value="1" <{if $setup.value=='1'}>checked<{/if}> ><{$smarty.const._YES}>
          </label>
          <label class="radio-inline">
            <input type="radio" name="<{$setup.name}>" id="<{$setup.name}>0" value="0" <{if $setup.value!='1'}>checked<{/if}> ><{$smarty.const._NO}>
          </label>
