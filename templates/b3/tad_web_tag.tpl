...
<{if $tag}>
  ...
  <{foreach from=$show_arr item=dirname}>
    <{if "$xoops_rootpath/modules/tad_web/plugins/`$dirname`/tpls/b3/tad_web_common_`$dirname`.tpl"|file_exists}>
      <div class="row">
        <div class="col-sm-12">
          <{includeq file="$xoops_rootpath/modules/tad_web/plugins/`$dirname`/tpls/b3/tad_web_common_`$dirname`.tpl"}>
        </div>
      </div>
    <{/if}>
  <{/foreach}>
