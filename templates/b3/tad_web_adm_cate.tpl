...
<div class="container-fluid">
  ...
  <{if $now_op=="tad_web_cate_form"}>
    ...
    <{foreach from=$all_content key=i item=cate}>
      <div id="tabs-<{$i}>">
        <{$cate.CateName}>
        <a href="javascript:delete_tad_web_cate_func(<{$cate.CateID}>);" class="btn btn-xs btn-danger"><{$smarty.const._TAD_DEL}></a>
        <a href="<{$xoops_url}>/modules/tad_web/admin/cate.php?op=tad_web_cate_form&CateID=<{$cate.CateID}>" class="btn btn-xs btn-warning"><{$smarty.const._TAD_EDIT}></a>

        <form action="cate.php" method="post" id="myForm<{$i}>" class="form-horizontal" role="form">
          <div class="form-group">
            <div class="col-sm-5">
