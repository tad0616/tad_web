<{if $block.op=="login"}>
  <{if $LoginMemNickName==""}>
    ...
    <div class="well">
      ...
      <form action="aboutus.php" method="post" enctype="multipart/form-data" class="form-horizontal" role="form" id="tad_web_login">
        <div class="form-group">
          <label class="col-sm-4 control-label">
          ...
          <{else}>
            <tr style="background-color: #dfdfdf;">
              ...
              <td colspan=3 style="background-color: #dfdfdf;">
                <a href="<{$xoops_url}>/modules/tad_web/config.php?WebID=<{$plugin.WebID}>&dirname=<{$plugin.dirname}>&op=enabe_plugin" class="btn btn-primary btn-xs"><{$smarty.const._MB_TCW_ENABLE_PLUGIN}></a>
              </td>
            </tr>
          <{/if}>
        <{/if}>
    ...
    <div class="row">
      <div class="col-sm-12">
        <{foreach from=$block.closed_webs item=web}>
          <a href="<{$web.url}>" class="btn btn-default btn-block"><{$smarty.const._MB_TCW_ENABLE}> <{$web.name}></a>
        <{/foreach}>
      </div>
    </div>
  <{/if}>
