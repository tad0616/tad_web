<{if $isMyWeb}>
  ...
  <form action="assistant.php" method="post" id="myForm" enctype="multipart/form-data" role="form">
    <table class="table">
      <tr>
        <td>
      ...
      <{if $all_assistant}>
        <{foreach from=$all_assistant item=assistant}>
          <tr>
            ...
            <td <{if $assistant.ColName==$plugin}>style="background-color:#F0F7AF;"<{/if}>>
              <a href="javascript:delete_assistant_func('<{$assistant.CateID}>');" class="btn btn-danger btn-sm"><{$smarty.const._TAD_DEL}></a>
            </td>
          </tr>
        <{/foreach}>
      <{/if}>
