...
<{elseif $web_display_mode=='index_plugin'}>
  <h1><a href="<{$xoops_url}>/modules/tad_web/"><i class="fa fa-home"></i></a> <{$video.PluginTitle}></h1>
...
<{if $video_data}>
  ...
  <thead>
    <tr>
      <th data-hide="phone" style="width:100px;text-align:center;">
        <{$smarty.const._MD_TCW_VIDEO_SHORT}>
      </th>
  ...
  <tbody id="sort">
    <{foreach item=video from=$video_data name=v}>
      <tr id="VideoID_<{$video.VideoID}>">
        ...
        <td>
          <p>
            <span class="label label-warning"><{$smarty.foreach.v.iteration}></span>
            <{$video.VideoDate}>
  ...
  <div style="text-align:right; margin: 0px 0px 10px;">
    <{if $web_display_mode=='index'}>
      <a href="video.php" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_VIDEO_SHORT}></a>
    <{elseif $web_display_mode=='home' or $VideoDefCateID}>
      <a href="video.php?WebID=<{$WebID}>" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_VIDEO_SHORT}></a>
    <{/if}>

    <{if $isMyWeb and $WebID}>
      <a href="video.php?WebID=<{$WebID}>&op=edit_form" class="btn btn-info <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-plus"></i> <{$smarty.const._MD_TCW_ADD}><{$smarty.const._MD_TCW_VIDEO_SHORT}></a>
    <{/if}>
  </div>
<{/if}>