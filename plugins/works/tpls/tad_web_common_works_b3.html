...
<{if $works_data}>
    ...
    <{foreach item=work from=$works_data}>
      <tr>
        ...
        <td>
          ...
          <{if $work.isMyWeb or $work.isAssistant}>
  ...
  <div style="text-align:right; margin: 0px 0px 10px;">
    <{if $web_display_mode=='index'}>
      <a href="works.php" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_WORKS_SHORT}></a>
    <{elseif $web_display_mode=='home' or $WorksDefCateID}>
      <a href="works.php?WebID=<{$WebID}>" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_WORKS_SHORT}></a>
    <{/if}>
