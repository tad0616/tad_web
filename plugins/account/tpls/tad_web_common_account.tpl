<{if $web_display_mode=='home_plugin'}>
  <h1><a href="index.php?WebID=<{$WebID}>"><i class="fa fa-home"></i></a> <a href="account.php?WebID=<{$WebID}>&CateID=<{$cate.CateID}>"><{$cate.CateName}></a> <{$account.PluginTitle}></h1>
<{/if}>
...
<{if $account_data}>
    ...
    <{foreach item=account from=$account_data}>
      <tr>
        ...
        <td>
          ...
          <{if $account.hide}><span class="label label-danger"><{$account.hide}></span><{/if}>
          <{if $account.isMyWeb or $account.isAssistant}>
  ...
  <div style="text-align:right; margin: 0px 0px 10px;">
    <{if $web_display_mode=='index'}>
      <a href="account.php" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_ACCOUNT_SHORT}></a>
    <{elseif $web_display_mode=='home' or $AccountDefCateID}>
      <a href="account.php?WebID=<{$WebID}>" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_ACCOUNT_SHORT}></a>
    <{/if}>
