<{if $op=="edit_form"}>
  ...
  <div class="card card-body bg-light m-1">
    <form action="files.php" method="post" id="myForm" enctype="multipart/form-data" role="form">
...
<{elseif $op=="list_all"}>
    ...
    <div class="row">
      ...
      <div class="col-md-4 text-right">
        <{if $isMyWeb and $WebID}>
          <a href="cate.php?WebID=<{$WebID}>&ColName=files&table=tad_web_files" class="btn btn-warning <{if $web_display_mode=='index'}>btn-sm<{/if}>"><i class="fa fa-folder-open"></i> <{$smarty.const._MD_TCW_CATE_TOOLS}></a>
          <a href="files.php?WebID=<{$WebID}>&op=edit_form" class="btn btn-info <{if $web_display_mode=='index'}>btn-sm<{/if}>"><i class="fa fa-plus"></i> <{$smarty.const._MD_TCW_ADD}><{$smarty.const._MD_TCW_FILES_SHORT}></a>
        <{/if}>
      </div>
    </div>
  <{/if}>
  <{if $file_data}>
    <{$FooTableJS}>
    <{includeq file="$xoops_rootpath/modules/tad_web/plugins/files/tpls/b4/tad_web_common_files.tpl"}>
  <{else}>
