...
<{elseif $web_display_mode=='index_plugin'}>
  <h1><a href="<{$xoops_url}>/modules/tad_web/"><i class="fa fa-home"></i></a> <{$link.PluginTitle}></h1>
...
<{if $link_data}>
      ...
      <{foreach item=link from=$link_data name=l}>
        <tr id="LinkID_<{$link.LinkID}>">
          <td>
            ...
            <span class="label label-warning"><{$smarty.foreach.l.iteration}></span>
            ...
            <{if $link.LinkDesc}>
              <div style="font-size:12px;color:#666699; line-height:1.5;"><{$link.LinkDesc}></div>
            <{/if}>
          </td>
  ...
  <div style="text-align:right; margin: 0px 0px 10px;">
    <{if $web_display_mode=='index'}>
      <a href="link.php" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_LINK_SHORT}></a>
    <{elseif $web_display_mode=='home' or $LinkDefCateID}>
      <a href="link.php?WebID=<{$WebID}>" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_LINK_SHORT}></a>
    <{/if}>

    <{if $isMyWeb and $WebID}>
      <a href="link.php?WebID=<{$WebID}>&op=edit_form" class="btn btn-info <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-plus"></i> <{$smarty.const._MD_TCW_ADD}><{$smarty.const._MD_TCW_LINK_SHORT}></a>
    <{/if}>
  </div>
<{/if}>