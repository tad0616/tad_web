<{if $web_display_mode=='index' and $page_data}>
    <{if "$xoops_rootpath/uploads/tad_web/0/image/`$dirname`.png"|file_exists}>
        <a href="<{$xoops_url}>/modules/tad_web/<{$dirname|default:''}>.php"><img src="<{$xoops_url}>/uploads/tad_web/0/image/<{$dirname|default:''}>.png" alt="<{$page.PluginTitle}>"></a>
    <{else}>
...
<{if $page_data|default:false}>
    ...
    <{foreach from=$cate_arr item=cate}>
        ...
        <{if $cate_data.$cid}>
            <h3>
                <a href="page.php?WebID=<{$WebID|default:''}>&CateID=<{$cid|default:''}>"><{$cate.CateName}></a>
    ...
    <{if $cate_data.0}>
        <h3><a href="page.php?WebID=<{$WebID|default:''}>&CateID=0"><{$smarty.const._MD_TCW_PAGE_UNCATEGORY}></a></h3>
    ...
    <div style="text-align:right; margin: 0px 0px 10px;">
        ...
        <{if $isMyWeb and $WebID}>
            <a href="page.php?WebID=<{$WebID|default:''}>&op=edit_form" class="btn btn-info <{if $web_display_mode=='index'}>btn-sm btn-xs<{/if}>"><i class="fa fa-plus"></i> <{$smarty.const._MD_TCW_ADD}><{$smarty.const._MD_TCW_PAGE_SHORT}></a>
        <{/if}>
    </div>
<{/if}>
