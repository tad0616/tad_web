...
<{elseif $web_display_mode=='index_plugin'}>
  <h1><a href="<{$xoops_url}>/modules/tad_web/"><i class="fa fa-home"></i></a> <{$discuss.PluginTitle}></h1>
  ...
  <{else}>
        ...
        <{foreach item=discuss from=$discuss_data}>
          <tr>
            ...
            <{if isset($discuss.cate.CateID)}>
              <span class="label label-info"><a href="discuss.php?WebID=<{$discuss.WebID}>&CateID=<{$discuss.cate.CateID}>" style="color: #FFFFFF;"><{$discuss.cate.CateName}></a></span>
            <{/if}>
    ...
    <div style="text-align:right; margin: 0px 0px 10px;">
      <{if $web_display_mode=='index'}>
        <a href="discuss.php" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_DISCUSS_SHORT}></a>
      <{elseif $web_display_mode=='home' or $DiscussDefCateID}>
        <a href="discuss.php?WebID=<{$WebID}>" class="btn btn-primary <{if $web_display_mode=='index'}>btn-xs<{/if}>"><i class="fa fa-info-circle"></i> <{$smarty.const._MD_TCW_MORE}><{$smarty.const._MD_TCW_DISCUSS_SHORT}></a>
      <{/if}>
