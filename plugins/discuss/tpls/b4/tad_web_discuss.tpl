...
<{elseif $op=="show_one"}>
    ...
    <div class="row">
        ...
        <div class="col-md-9 col-lg-10">
            ...
            <div style="float: right;">
              <{if $isMineDiscuss}>
                <a href="javascript:delete_discuss_func(<{$DiscussID}>);" class="btn btn-sm btn-danger"><{$smarty.const._TAD_DEL}></a>
                <a href="discuss.php?WebID=<{$WebID}>&op=edit_form&DiscussID=<{$DiscussID}>" class="btn btn-sm btn-warning"><{$smarty.const._TAD_EDIT}></a>
              <{/if}>
            </div>
        </div>
    </div>
  ...
  <{if $LoginMemID or $LoginParentID or $isMineDiscuss}>
    ...
    <form action="discuss.php" method="post" id="myForm" enctype="multipart/form-data" role="form" style="margin-top:16px;">
      <div class="form-group row">
        <div class="col-md-12">
    ...
    <div class="row">
      ...
      <div class="col-md-4 text-right">
        <{if $isMyWeb and $WebID}>
          <a href="cate.php?WebID=<{$WebID}>&ColName=discuss&table=tad_web_discuss" class="btn btn-warning <{if $web_display_mode=='index'}>btn-sm<{/if}>"><i class="fa fa-folder-open"></i> <{$smarty.const._MD_TCW_CATE_TOOLS}></a>
        <{/if}>

        <{if $isMyWeb or $LoginMemID or $LoginParentID}>
          <a href="discuss.php?WebID=<{$WebID}>&op=edit_form" class="btn btn-info <{if $web_display_mode=='index'}>btn-sm<{/if}>"><i class="fa fa-plus"></i> <{$smarty.const._MD_TCW_DISCUSS_ADD}></a>
        <{/if}>
      </div>
    </div>
  <{/if}>
