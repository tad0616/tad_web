...
class power
{
    ...
    public function power_menu($power_name = "read", $col_name = "", $col_sn = "")
    {
        ...
        $select_users     = $power_val == 'users' ? 'selected' : '';
        $select_web_users = $power_val == 'web_users' ? 'selected' : '';
        $select_web_admin = $power_val == 'web_admin' ? 'selected' : '';
    ...
    public function check_power($power_name = "", $col_name = "", $col_sn = "")
    {
        ...
        if ($power == "users" and !$xoopsUser and empty($LoginWebID)) {
            // die("沒有登入");
            return false;
        } elseif ($power == "web_users" and $LoginWebID != $this->WebID and !$isMyWeb) {
            // die("非本站使用者" . $LoginWebID . "!=" . $this->WebID);
            return false;
        } elseif ($power == "web_admin" and !$isMyWeb) {
            return false;
        }
        return true;
    }
